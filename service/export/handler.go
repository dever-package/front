...
func CreateTask(c *server.Context) error {
	...
	if err := permissionservice.EnsureActionAccessWithInput(c.Context(), pagePath, exportKey, exportInputLookup(c)); err != nil {
		return c.JSONPayload(http.StatusForbidden, map[string]any{
			"code":   http.StatusForbidden,
			"status": 2,
			"msg":    err.Error(),
...
func exportInputLookup(c *server.Context) permissionservice.InputLookup {
